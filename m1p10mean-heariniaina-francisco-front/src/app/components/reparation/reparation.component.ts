...
export class ReparationComponent implements OnInit {
  ...
  voitureDeposes(id: any) {
    this.http.get("http://51.178.17.54:3001/api/list/" + id + "/reparation-client-attente").subscribe((result: any) => {
      this.data = result;
  ...
  // avancement(id: any) {
  //   // alert(id)
  //   this.http.get("http://51.178.17.54:3001/api/etat-avancement/"+id+"/reparation-client").subscribe((result: any) => {
  //     this.data = result;
